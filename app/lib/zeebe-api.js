...
module.exports = class ZeebeAPI {
  ...
  async checkConnectivity(parameters) {
    ...
    try {
      ...
      log.error(`Failed to connect with config: ${ withoutSecrets(parameters) }`, err);
  ...
  async deploy(parameters) {
    ...
    const { contents } = this.fs.readFile(parameters.filePath, { encoding: 'buffer' });
    ...
    try {
      ...
      log.error(`Failed to deploy with config: ${ withoutSecrets(parameters) }`, err);
  ...
  async run(parameters) {
    ...
    try {
      ...
      log.error(`Failed to run instance with config: ${ withoutSecrets(parameters) }`, err);
